...
| -------- | -------------- |
| [payload-start/description.md](payload-start/description.md) | Bootstrapping a new Payload site (target: **Dokploy on Hetzner**): upload limits, WebP/Sharp resize, **`@payloadcms/storage-s3`**, proxy/body limits for large files, then experimental two-step Next.js build. ⚠️ Todo 3 (two-step build) is superseded by `static-rendering` — keep it only as a fallback for when the build cannot reach the database. |
