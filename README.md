...
| [security-check/description.md](security-check/description.md) | Pre-deployment security checklist for Next.js, Payload, and hosting: HTTP headers, env/secrets, CORS, rate limiting, file uploads, admin hardening, infrastructure basics, and **supply-chain security** — `pnpm audit` deploy gate in `nixpacks.toml` (start `critical`, raise to `high`), `.github/dependabot.yml` with Payload-lockstep grouping, and Payload advisory patching. |
