...
| [static-rendering/description.md](static-rendering/description.md) | Getting **real static rendering (SSG)** out of Payload + Next.js on Dokploy: why the two-step build switches the route cache off entirely, `generateStaticParams` across a `[locale]` parent segment, how middleware rewrites drop runtime renders out of the cache, locale-aware `revalidatePath`, and splitting `DATABASE_URI` between build and runtime. With measured numbers. |
